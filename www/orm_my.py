	...
	def __new__(cls, name, bases, attrs):
		...
		for k, v in attrs.items():
			if isinstance(v, Field):
				...
				if v.primary_key:
					if primaryKey:
						raise RuntimeError("Duplicate primary key filed: %s" % k)
					primaryKey = k
				else:
					fields.append(k)
		if not primaryKey:
		...
		attrs["__primary_key__"] = primaryKey  # 主键名字
		attrs["__fields__"] = fields  # 除主键外的属性名
		# 构建默认的Select,insert, update, delecte语句
